...
! ---- cntr of D.B.L ---------------
  ...
  ymin = minval(y) - (maxval(y)-minval(y))*0.1d0
  ymax = maxval(y) + (maxval(y)-minval(y))*0.1d0
  zmin = minval(dudt) - (maxval(dudt)-minval(dudt))*0.1d0
  zmax = maxval(dudt) + (maxval(dudt)-minval(dudt))*0.1d0
  zmin = -0.0002d0
  zmax =  0.0002d0
  ...
  call set_window(xmin, xmax, ymin, ymax, trim(axis_b), trim(axis_l), trim(axis_t), pxmin, pxmax, pymin, pymax)
  call set_clr_cntr(clrnm, zmin, zmax)
  call plot_cntr(1, iend, 1, jend, x, y, dudt)
  call plcol0(9)
  call pljoin(x(1,jend-2),y(1,jend-2),x(iend,jend-2),y(iend,jend-2))
  call colbar(0.d0, 1.d0, zmin, zmax, pxmax+0.02d0, pxmax+0.03d0, pymin, pymax, trim(barsname))
!------------------------------------------
...
! ---- cntr of D.B.L ---------------
  ...
  ymin = minval(y) - (maxval(y)-minval(y))*0.1d0
  ymax = maxval(y) + (maxval(y)-minval(y))*0.1d0
  zmin = minval(dudx) - (maxval(dudx)-minval(dudx))*0.1d0
  zmax = maxval(dudx) + (maxval(dudx)-minval(dudx))*0.1d0
  zmin = -0.2d0
  zmax =  0.2d0
  ...
  call set_window(xmin, xmax, ymin, ymax, trim(axis_b), trim(axis_l), trim(axis_t), pxmin, pxmax, pymin, pymax)
  call set_clr_cntr(clrnm, zmin, zmax)
  call plot_cntr(1, iend, 1, jend, x, y, dudx)
  call plcol0(9)
  call pljoin(x(1,jend-2),y(1,jend-2),x(iend,jend-2),y(iend,jend-2))
  call colbar(0.d0, 1.d0, zmin, zmax, pxmax+0.02d0, pxmax+0.03d0, pymin, pymax, trim(barsname))
!------------------------------------------
...
! ---- lng ---------------
  ...
  xmin = minval(x) - (maxval(x)-minval(x))*0.02d0
  xmax = maxval(x) + (maxval(x)-minval(x))*0.02d0
  ! ymin = minval(dudx) - (maxval(dudx)-minval(dudx))*0.1d0
  ! ymax = maxval(dudx) + (maxval(dudx)-minval(dudx))*0.1d0
  ymin = -0.2d0
  ymax =  0.2d0
  ...
  call set_window(xmin, xmax, ymin, ymax, trim(axis_b), trim(axis_l), trim(axis_t), pxmin, pxmax, pymin, pymax)
  call plot_line(iend, x(:,1), dudx(:,jend-2), 9)
  call plot_line(iend, x(:,1), dudt(:,jend-2), 1)
