...
! ---- lng of B.L ---------------
  ...
  xmin = minval(x) - (maxval(x)-minval(x))*0.02d0
  xmax = maxval(x) + (maxval(x)-minval(x))*0.02d0
  ymin = minval(bl(:,jend-2)) - (maxval(bl(:,jend-2))-minval(bl(:,jend-2)))*0.1d0
  ymax = maxval(bl(:,jend-2)) + (maxval(bl(:,jend-2))-minval(bl(:,jend-2)))*0.1d0
!!$  ymin = 0.04d0 ; ymax = 0.06d0
!!$  
  pxmin = 0.65d0 ; pxmax = 0.95d0
  pymin = 0.82d0 ; pymax = 0.95d0  
  ...
  call set_window(xmin, xmax, ymin, ymax, trim(axis_b), trim(axis_l), trim(axis_t), pxmin, pxmax, pymin, pymax)
  call plot_line(iend, x(:,jend-2), bl(:,jend-2), 1)
  !------------------------------------------
  ...
  xmin = minval(x) - (maxval(x)-minval(x))*0.02d0
  xmax = maxval(x) + (maxval(x)-minval(x))*0.02d0
  ymin = minval(ibi(:,jend-2)) - (maxval(ibi(:,jend-2))-minval(ibi(:,jend-2)))*0.1d0
  ymax = maxval(ibi(:,jend-2)) + (maxval(ibi(:,jend-2))-minval(ibi(:,jend-2)))*0.1d0
  write(*,*)sum(ibi(:,:))/(iend*jend)
  ...
  xmin = minval(x) - (maxval(x)-minval(x))*0.02d0
  xmax = maxval(x) + (maxval(x)-minval(x))*0.02d0
  ymin = minval(dif_dep(:,jend-2)) - (maxval(dif_dep(:,jend-2))-minval(dif_dep(:,jend-2)))*0.1d0
  ymax = maxval(dif_dep(:,jend-2)) + (maxval(dif_dep(:,jend-2))-minval(dif_dep(:,jend-2)))*0.1d0
  ymin = 0.0d0 ; ymax = 100.0d0

  pymin = pymin - pdy ; pymax = pymax - pdy
    
  call set_window(xmin, xmax, ymin, ymax, trim(axis_b), trim(axis_l), trim(axis_t), pxmin, pxmax, pymin, pymax)
  call plot_line(iend, x(:,jend-2), dif_dep(:,jend-2), 1)
  !------------------------------------------
